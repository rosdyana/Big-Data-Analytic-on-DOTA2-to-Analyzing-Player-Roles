...
# initiator = c(2,7,13,16,29,38,51,55,65,97,100,103)
# nuker = c(22,45,98,105)
# escape = c(62)
# disabler = c(88,14,33)
...
# merge the data
#matchesData <- rbind(TI2014, TI2015, TI2016)
matchesData <- read.csv("TI2016.csv", header = T, sep = ",")

matchesId <- unique(matchesData$match_id)
# x = get_match_details(2569610900)
# x$players$tower_kills
# class = ""
# if(hero %in% carry ) class = "carry"
# if(hero %in% support) class = "support"
# if(hero %in% escape ) class = "support"
# if(hero %in% initiator) class = "ganker"
# if(hero %in% nuker ) class = "ganker"
# if(hero %in% disabler ) class = "support"
# if(TK > 5 || LH > 300) class = "pusher"
# if(death > 10) class = "feeder"
# dataset format 
# accountid ,level, kill, death, assist , neutral kill,gem, GPM, XPM, last hit, denied, Hero damaged, hero healing, tower damage,tower kill, gold, XP, hero as class
for ( i in matchesId )
{
  print(i)
